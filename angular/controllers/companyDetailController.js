angular.module("companyDetailController", ['angularjs-dropdown-multiselect']).controller("companyDetailController", 
function($scope, $rootScope, $http, companyService, $window, $document, $timeout) {
	...
	$rootScope.startInstanceOptionDay = []; 
	$rootScope.startInstanceOptionData = [ {id: 'MON', label: "Monday"}, {id: 'TUE', label: "Tuesday"}, {id: 'WED', label: "Wednesday"},, {id: 'THU', label: "Thursday"}, {id: 'FRI', label: "Friday"}, {id: 'SAT', label: "Saturday"}, {id: 'SUN', label: "Sunday"} ];
	
	$rootScope.stopInstanceOptionDay = []; 
	$rootScope.stopInstanceOptionData = [ {id: 'MON', label: "Monday"}, {id: 'TUE', label: "Tuesday"}, {id: 'WED', label: "Wednesday"},, {id: 'THU', label: "Thursday"}, {id: 'FRI', label: "Friday"}, {id: 'SAT', label: "Saturday"}, {id: 'SUN', label: "Sunday"} ];
...
//		}).
	  ...
	  var getUTCTime = function(hour) {	  

			var today = new Date();
			var currentDate = today.getFullYear()+'-'+(today.getMonth()+1)+'-'+today.getDate();
			var NewDate = currentDate + " "+ hour;
		...
		if(value == "project_ok"){
			...
			if(pname == undefined || pname.trim().length <= 0){
				...
				$http({
					method : "POST",
					url : "/createProject",
					data : {pname : pname.trim(), cid : $rootScope.companyServiceId},
					headers : {"Content-Type" : "application/json"}
				}).
				success(function(data){
					if(data.success == 1){
						var result = {projectName: pname.trim(), id: data.row_id, company_id: $rootScope.companyServiceId, Liveservers: liveserverdata, servers:serverdata};
			...
			if(pname == undefined || pname.trim().length <= 0){
				...
				$http({
					method : "POST",
					url : "/editProject",
					data : {pname : pname.trim(), cid : $rootScope.companyServiceId,pid:pid},
					headers : {"Content-Type" : "application/json"}
				}).
				success(function(data){
					if(data.success == 1){
						var result = {projectName: pname.trim(), id: pid, company_id: $rootScope.companyServiceId, Liveservers: liveserverdata, servers:serverdata};
						var companydata = $rootScope.companies;
						for(var x in companydata){
							var projects = companydata[x].projects;
							if(result.company_id == companydata[x].id){
		    ...
		    success(function(data){
					 ...
					 if($rootScope.startInstanceOptionDay.length == 0){
                        alert("Please select the day to start the instance");
                        return false;
                     }
					 if($rootScope.stopInstanceOptionDay.length == 0){
                        alert("Please select the day to stop the instance");
                        return false;
                     }
                    ...
                    body.removeClass("overflowHidden");
                    $rootScope.modal_class = "modal-backdrop fade in loader";
                    var startDay = $rootScope.startInstanceOptionDay;
					startDay = startDay.map(function(a) {return a.id;});
					startDay = startDay.toString();
                    var stopDay = $rootScope.stopInstanceOptionDay;
					stopDay = stopDay.map(function(a) {return a.id;});
					stopDay = stopDay.toString();				
                    $http({
                            method : "POST",
			    url : "/createSchedulerTask",
                            data: {projectId :$rootScope.projectId,startDay:startDay,stopDay:stopDay,startTime :$rootScope.startInstanceTime,stopTime :$rootScope.stopInstanceTime,startUTCTime:getUTCTime($rootScope.startInstanceTime), stopUTCTime:getUTCTime($rootScope.stopInstanceTime)},
			    headers : {"Content-Type" : "application/json"}
		    }).
		    success(function(data){
                        $rootScope.modal_class = "";
			    ...
			    url : "/editSchedulerTask",
                            data: {projectId :$rootScope.projectId,startDay:startDay,stopDay:stopDay,startTime :$rootScope.startInstanceTime,stopTime :$rootScope.stopInstanceTime,sc1:$rootScope.sc1,sc2:$rootScope.sc2,startUTCTime:getUTCTime($rootScope.startInstanceTime), stopUTCTime:getUTCTime($rootScope.stopInstanceTime)},
			    headers : {"Content-Type" : "application/json"}
		    }).
		    success(function(data){
                        $rootScope.modal_class = "";
