angular.module("automationController", []).controller("automationController",
        function ($scope, $rootScope, $http, companyService, $window, $document, $timeout) {
            ...
            $rootScope.editScriptModel = function (script_id,script_name,script_desc,script,companyID){
                ...
                if (value == "create_script_cancel") {
                   ...
                   /* } else if (countScriptLine > 1 && parameter.length > 0) {
                        ...
                        $http({
                            method: "POST",
                            url: "/createScript",
                            data: {script_name: script_name, script_desc: script_desc, script: script,  parameter:parameter,companyid:uCompanyId},
                            headers: {"Content-Type": "application/json"}
                        }).success(function (data) {
                            if (data.success == 1) {
                                ...
                                body.removeClass("overflowHidden");
                                $rootScope.modal_class = "";
								alert('The script has been created');
								location.reload();
                   ...
                   /* } else if (countScriptLine > 1 && parameter.length > 0) {
                        ...
                        $http({
                            method: "POST",
                            url: "/editScript",
                            data: {script_id:script_id, script_desc: script_desc, script: script, parameter: parameter, parameter_name: parameter_name,companyid:uCompanyId},
                            headers: {"Content-Type": "application/json"}
                        }).success(function (data) {
                            if (data.success == 1) {
                                location.reload();
