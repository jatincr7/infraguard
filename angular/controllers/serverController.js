...
app.controller("serverController",
    function ($scope, $http, $rootScope, companyService, $window, $timeout, $document) {
        ...
        $rootScope.lastweekdate = new Date($rootScope.today.getTime() - (7 * 24 * 60 * 60 * 1000));
        $rootScope.lastmonthdate = new Date($rootScope.today.getTime() - (31 * 24 * 60 * 60 * 1000));
     ...
     $rootScope.deleteParameter = function (parIndex) {
        ...
        $rootScope.selectCustomer = function (customerId, servers) {
            $(".servers_" + customerId).prop("checked", true);
            var serverId = [];
            var y = 0;
            for (var x in servers) {
                if (servers[x].ssm_status == 'Alive' || servers[x].ssm_status == 'Online') {
                    serverId[y] = servers[x].id;
                    y++;
                }
            }
            $rootScope.serverIDs[customerId] = serverId;
            var idx = $rootScope.customerIds.indexOf(customerId);
            if (idx == -1) {
                $(".server_" + customerId).css("display", "block");
                $rootScope.customerIds.push(customerId);
            } else {
                $(".server_" + customerId).css("display", "none");
                $rootScope.customerIds.splice(idx, 1);
                $rootScope.serverIDs[customerId] = [];
            }
        };
        $rootScope.selectServer = function (serverId, customers) {
            var idx = $rootScope.serverIDs[customers].indexOf(serverId);
            if (idx == -1) {
                $rootScope.serverIDs[customers].push(serverId);
            } else {
                $rootScope.serverIDs[customers].splice(idx, 1);
            }
            if ($rootScope.serverIDs[customers].length == 0) {
                $(".server_" + customers).css("display", "none");
                $(".customer_" + customers).prop("checked", false);
                $(".servers_" + customers).prop("checked", true);
                for (var x in $rootScope.customerIds) {
                    if ($rootScope.customerIds[x] == customers) {
                        $rootScope.customerIds[x] = [];
                    }
                }
            }
        };
        $rootScope.runScriptForProject = function (servers, projectId, script_id, script_name, script, script_desc) {
            ...
            $rootScope.customerIds.length = 0;
            $rootScope.serverIDs.length = 0;
            $rootScope.parameters = [];
                                ...
                                for (var y in data) {
                                    if (server_id[x] == data[y].server_id) {
                                        if (j < 5) {
                                            ...
                                            if (data[y].last_rotate_date != null) {
                                                var dateString = data[y].last_rotate_date;
                                                last_rotate_date = Date.parse(dateString);
                                            } else {
                                                last_rotate_date = new Date().getTime();
                                            }

                                            userDetails[j] = { "username": userName[j], "id": userId[j], "email": userEmail[j], "private_key": userPrivateKey[j], "last_rotate_date": last_rotate_date };
                                            j++;
        ...
        $rootScope.close = function (value) {
            if (value == "user_ok") {
                ...
                $rootScope.emailError = false;
                $rootScope.chkConfirmError = false;
                if ($rootScope.serverkeyEmail == "" || $rootScope.serverkeyEmail == undefined || $rootScope.serverkeyEmail == null) {

                    $rootScope.emailError = true;
                    return false;
                } else if (emailPattern.test($rootScope.serverkeyEmail) == false) {
                    $rootScope.emailError = true;
                    return false;
                } else if ($('#sendUpdateServerUsersKey').prop('checked') == false) {

                    $rootScope.chkConfirmError = true;
                    return false;
                } else {
                    ...
                    $http({
                        ...
                        method: "POST",
                        data: { projectId: id, serverId: $rootScope.serverId, sendEmailStatus: sendEmailStatus, emailTo: $rootScope.serverkeyEmail },
                        headers: { "Content-Type": "application/json" }
                    })
                        .success(function (data) {
                        });
                ...
                $rootScope.serverIds = [];
                var y = 0;
                for (var x in $rootScope.serverIDs) {
                    if ($rootScope.serverIDs[x] != "") {
                        $rootScope.serverIds[y] = $rootScope.serverIDs[x];
                        y++;
                    }
                }
        ...
        $rootScope.createDropdowns = function () {
